...
date: " created 4 October 2021, updated 20 March 2026"
...
unique(Western_Europe$Entity)
Western_Europe %>% 
  ggplot(aes(x = Year, y = homocides_per_100k, color = Country))+
  geom_line()+
  theme_minimal()+
...
Western_Europe %>% 
  ...
  facet_wrap(~ Country)+
  labs(title = "Long-term Trends in Homicide Rates in Europe")
...
kings %>% 
  ...
  mutate(midyear = End_of_Reign - duration/2) %>% 
  ggplot(aes(x = midyear, y = duration))+
  geom_point(color = "blue")+
  geom_smooth()+
  theme_classic()+
  labs(title = "Duration of Danish Monarchs reign over time",
...
The plots visualize presents long-term trends in homicide rates across selected European countries, illustrating a general decline from the early modern period to the present. In earlier centuries, particularly between the 1300s and 1600s, homicide rates were significantly higher, in some cases even exceeding 40-70 per 100,000 inhabitants. Over time, these rates steadily decrease, reaching close to zero by the modern period.  The pattern suggests that European societies have become less violent over time, which can be seen as evidence, that we have become more civilized today. The consistent downward trajectory across these different countries strengthens this conclusion, indicating that the change is not an isolated event, but part of a broader historical transformation. Possible explanations to this could be stronger state institutions, improved legal systems and changing of social norms that discourage violence. 
However, the visualizations only capture homicide – which is just one single aspect of violence. This provides us with a limited perspective, where other types of violence, such as war, structural violence, or crimes that does not result in death are not considered.
The data strongly suggest a major decline in interpersonal violence and support the idea of societies becoming more civilized today, while one must also keep in mind the limitations of the data.
